...
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_readMetainfoNative(JNIEnv *env, jobject thiz,
                                                                          jobject javaBook) {
    std::shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
    if (!plugin) {
        return 1;
    }
...
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_readEncryptionInfosNative(JNIEnv *env,
                                                                                 jobject thiz,
                                                                                 jobject javaBook) {
    std::shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
    if (!plugin) {
...
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_readUidsNative(JNIEnv *env, jobject thiz,
                                                                      jobject javaBook) {
    std::shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
    if (!plugin) {
        return;
...
extern "C"
JNIEXPORT void JNICALL
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_detectLanguageAndEncodingNative(JNIEnv *env,
                                                                                       jobject thiz,
                                                                                       jobject javaBook) {
    std::shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
    if (!plugin) {
...
static bool initInternalHyperlinks(JNIEnv *env, jobject javaModel, BookModel &model,
    ...
    ZLCachedMemoryAllocator allocator(131072, cacheDir, "nlinks");  //128K
...
extern "C"
JNIEXPORT jint JNICALL
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_readModelNative(JNIEnv *env, jobject thiz,
                                                                       jobject javaModel,
    ...
    shared_ptr<Book> book = Book::loadFromJavaBook(env, javaBook);
    std::shared_ptr<BookModel> model = (std::shared_ptr<BookModel>) new BookModel(book, javaModel,
                                                                                  cacheDir);
...
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_readAnnotationNative(JNIEnv *env,
                                                                            jobject thiz,
                                                                            jobject file) {
    std::shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
    if (!plugin) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_koolearn_kooreader_formats_NativeFormatPlugin_readCoverNative(JNIEnv *env, jobject thiz,
                                                                       jobject file,
                                                                       jobjectArray box) {
    std::shared_ptr<FormatPlugin> plugin = findCppPlugin(thiz);
    if (!plugin) {

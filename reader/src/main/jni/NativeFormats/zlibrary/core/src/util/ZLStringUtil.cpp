...
bool ZLStringUtil::parseChapter(std::string &baseString) {
	std::string first = baseString.substr(0, 1);
	std::string last = baseString.substr(baseString.length() - 1, 1);
	if (first == "第" && last == "章") return true;

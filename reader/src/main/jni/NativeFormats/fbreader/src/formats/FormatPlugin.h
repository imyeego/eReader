...
public:
	std::vector<std::shared_ptr<FormatPlugin> > plugins() const;
	std::shared_ptr<FormatPlugin> pluginByType(const std::string &fileType) const;
...
private:
	...
	std::vector<std::shared_ptr<FormatPlugin> > myPlugins;
};
...
inline std::vector<std::shared_ptr<FormatPlugin> > PluginCollection::plugins() const {
	return myPlugins;
}

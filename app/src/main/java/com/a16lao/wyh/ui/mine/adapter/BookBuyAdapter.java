...
public class BookBuyAdapter extends SimpleBaseAdapter<String> {
    protected Context mContext;
    private List<String> mList;

    public BookBuyAdapter(Context mContext, List<String> list) {
    ...
    protected void bindView(SimpleBaseViewHolder holder, String s, int position) {
        ...
        mList.add("1");
        mList.add("1");
        list_book_buy_item.setAdapter(new BookBuy2Adapter(mContext, mList, true));
        list_book_buy_item.setOnItemClickListener((parent, view, position1, id) -> {
//            if (SPUtils.getAppFlag(SPUtils.IS_LOGIN)) {
            mContext.startActivity(new Intent(mContext, BookBuyDetailActivity.class));
//            } else {
